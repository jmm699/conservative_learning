...
int *indeg,**inedge,**innode,*outdeg,**outedge,**outnode;
int hp, hm; //Stores how many nodes are in H+ and H-
...
outnode=malloc(nodes*sizeof(int*));//List of outgoing edges
...
for(i=0;i<nodes-classnodes;++i)
  ...
  outnode[i]=malloc(outdeg[i]*sizeof(int));
  outdeg[i]=0;
  }
...
for(d=0;d<indeg[i];++d)
  {
  in=innode[i][d];
  outnode[in][outdeg[in]] = i;
  outedge[in][outdeg[in]++]=inedge[i][d];
  }
//Mark the size of H+ and H-
//Assumption: all data nodes point to each node in H-,
//and all nodes in H+ point to all class nodes
hp = indeg[nodes-classnodes];
hm = outdeg[0];
